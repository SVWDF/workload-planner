    ...
    public class TicketsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<TicketDTO>>> GetScrumboardTickets(int scrumboardId)
        {
            try
            {
                ...
                var tickets = await _ticketService.GetScrumboardTicketsAsync(scrumboardId, user.Id);
                return Ok(tickets);                
            }
        ...
        public async Task<ActionResult<TicketDTO>> UpdateTicket(int id, UpdateTicketDTO dto)
        {
            ...
            {
                var user = await GetUserAsync();
                var ticket = await _ticketService.UpdateTicketAsync(id, dto, user.Id);
                return Ok(ticket);
            }
            catch (UserNotFoundException)
            {
        ...
        public async Task<ActionResult<TicketDTO>> AssignSelf(int id)
        {
            ...
            {
                var user = await GetUserAsync();
                var ticket = await _ticketService.AssignSelfToTicketAsync(id, user.Id);
                return Ok(ticket);
            }
            catch (UserNotFoundException)
            {
        ...
        public async Task<ActionResult<TicketDTO>> UpdateStatus(int id, UpdateTicketStatusDTO dto)
        {
            ...
            {
                var user = await GetUserAsync();
                var ticket = await _ticketService.UpdateStatusAsync(id, dto, user.Id);
                return Ok(ticket);
            }
            catch (UserNotFoundException)
            {
